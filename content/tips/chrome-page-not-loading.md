...
很多用户用过 VPN 或代理工具后没有彻底退出，Chrome 的系统代理设置还留着。关闭了 VPN 软件但系统代理没还原，Chrome 所有请求都走一个已经不通的代理通道，自然打不开任何网页。
...
DNS 是把域名（如 google.com）翻译成 IP 地址的服务。DNS 出了问题，Chrome 知道你要访问什么网站但找不到服务器在哪。
...
如果你经常遇到 [Chrome 浏览器](/) 的各种问题，建议收藏这篇文章备用。[Chrome 插件](/)也可能导致网络异常，排查时别忘了检查扩展程序。更多 Chrome 排查教程可以看 [Chrome 无痕模式详解](/tips/chrome-incognito-guide/) 和 [Chrome 下载速度慢怎么解决](/tips/chrome-download-speed/)。

相关推荐：[Chrome 隐私设置完全指南](/tips/chrome-privacy-settings/) · [Chrome 缓存清理教程](/tips/chrome-clear-cache/)
