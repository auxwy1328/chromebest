...
\[#国内用户要不要开启安全 DNS](/images/tips/chrome-secure-dns-setup/body3.jpg)
...
3. **国内 DNS 服务已经够用**：运营商提供的 DNS 和国内公共 DNS（如阿里 DNS、腾讯 DNS）已经足够快和安全。如果网页加载仍然慢，参考 [Chrome 网页打不开或加载慢排查指南](/tips/chrome-page-not-loading/)。
...
\[#系统 DNS 设置（和 Chrome ](/images/tips/chrome-secure-dns-setup/body2.jpg)
...
国内用户遇到的 DNS 劫持、污染等问题参考 [Chrome 在国内使用的常见问题与解决方案](/tips/chrome-common-problems-in-china/)。


## 国内 DNS 环境的特殊问题

\[#国内 DNS 环境的特殊问题](/images/tips/chrome-secure-dns-setup/body1.jpg)
...
安全 DNS 只加密了 DNS 查询这一步（隐藏了"你想访问哪个域名"），但实际的网页访问过程（你和网站服务器之间的数据传输）仍然是明文的。VPN 则加密了所有网络流量（包括 DNS 查询和实际的网页数据传输），提供的是端到端的隐私保护。VPN 的隐私保护远强于安全 DNS。但如果只是想在 DNS 层面增加一些隐私保护，安全 DNS 比 VPN 轻量得多（不影响网速，不增加额外费用）。Chrome 代理设置的完整方法参考 [Chrome 代理设置完整指南](/tips/chrome-proxy-settings/)。
