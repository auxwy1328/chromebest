...
**原因：** 某个扩展程序存在 bug，与网页的 JavaScript 代码冲突导致浏览器进程崩溃。如果你不确定是哪个扩展出的问题，可以参考 [Chrome 扩展程序故障排查指南](/tips/chrome-extensions-not-working/)。
...
**原因：** GPU 硬件加速与显卡驱动不兼容。这在更新显卡驱动后或使用老旧显卡时经常发生。关于硬件加速的详细原理和故障排查，参考 [Chrome 硬件加速设置教程](/tips/chrome-hardware-acceleration/)。
...
更详细的内存优化方法参考 [Chrome 内存占用优化教程](/tips/chrome-memory-optimization/)，10 种方法实测从 2.1GB 降到 1.3GB。
...
## 使用 chrome://crashes 查看崩溃详情

Chrome 内置了一个崩溃报告页面，记录了每次崩溃的详细信息。大多数用户不知道这个页面的存在，但它是最有用的崩溃诊断工具之一。

在地址栏输入 `chrome://crashes`，你会看到一张崩溃记录列表，包含以下信息：

- **崩溃时间**：精确到秒的崩溃时间戳
- **崩溃 ID**：由 Chrome 生成的唯一标识符，提交给 Google 后可用于追踪
- **上传状态**：是否已将崩溃报告上传到 Google 服务器进行分析
- **文件路径**：本地储存的崩溃转储文件（.dmp）的位置

### 如何利用崩溃 ID 排查问题

1. 在 `chrome://crashes` 页面找到最近一次崩溃
2. 点击「发送」按钮将崩溃报告上传到 Google（需要网络连接且能访问 Google 服务）
3. 记录崩溃 ID，在 Chrome 帮助论坛（support.google.com/chrome）搜索该 ID 或描述崩溃场景
4. 如果无法访问 Google 服务，可以在本地分析崩溃转储文件（见下文）

### 本地分析崩溃转储文件（Windows）

Chrome 的崩溃转储文件默认保存在 `%LOCALAPPDATA%\Google\Chrome\User Data\Crashpad\reports` 目录下。这些 `.dmp` 文件可以用 Windows 自带的 WinDbg 工具打开分析，不过这需要一定的技术背景。

对于普通用户，更实际的用法是：打开 `chrome://crashes` → 截图保存崩溃记录 → 发给技术支持或在论坛求助时附上崩溃 ID。这个信息比"Chrome 崩了"有用一百倍——技术人员看到崩溃 ID 可以直接定位到具体的崩溃原因和冲突模块。

## 预防 Chrome 崩溃的日常维护清单

与其等到崩溃再修复，不如养成几个习惯让 Chrome 保持稳定。以下是经过验证的日常维护清单：

### 每周一次：扩展程序审查

打开 `chrome://extensions/`，快速扫一遍已安装的扩展：
- 有没有不认识的扩展？可能是恶意软件偷偷安装的
- 有没有几个月没用过的扩展？卸载掉减少冲突风险
- 有没有显示"可能不再受支持"的扩展？尽早找替代品

扩展越多，崩溃风险越高。保留 10 个常用扩展比装 30 个偶尔用一两次的扩展安全得多。

### 每月一次：缓存和站点数据清理

参考 [Chrome 缓存清理教程](/tips/chrome-clear-cache/)，每月清理一次缓存（不勾选 Cookie）。这个频率既不会频繁到反复登录，也不会让缓存膨胀到影响稳定性。

### 显卡驱动更新时注意兼容性

每次更新显卡驱动后，打开 `chrome://gpu` 检查 GPU 加速状态。如果看到红色或黄色的警告项，优先关闭硬件加速（`chrome://settings/system`），等 NVIDIA/AMD/Intel 发布驱动修复后再重新开启。

### 避免"从不关闭 Chrome"

很多人把 Chrome 当成永不关闭的工具——开机自启动，永远不退出，标签页越积累越多。这对稳定性非常不利。建议每周完整退出 Chrome 一次（三个点 → 退出，而不是点 X 关窗口），让 Chrome 释放全部内存和文件句柄，重新启动后状态会干净很多。

### 关注 Chrome 版本更新

Chrome 的版本更新经常包含崩溃修复。打开 `chrome://settings/help` 确认自动更新正常工作。如果之前关闭了自动更新，建议重新开启——[Chrome 更新失败](/tips/chrome-update-failed/) 有详细的排查步骤。
...
Chrome 崩溃后重新打开时，通常会自动恢复崩溃前的标签页。如果没有自动恢复：点击三个点 → 历史记录 → 最近关闭的标签页 → 手动恢复。更多恢复方法参考 [Chrome 标签页丢失恢复指南](/tips/chrome-tabs-lost-recover/)。如果还是找不到，检查 Chrome 设置 → 启动时 → 是否勾选了"继续浏览上次打开的网页"。
