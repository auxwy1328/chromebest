...
- **广告拦截**：[Chrome 广告拦截插件推荐](/plugins/chrome-ad-blocker-extension-recommendation/) 能减少页面加载的脚本数量，间接降低 CPU/GPU 压力
- **扩展管理**：[Chrome 必装插件推荐](/plugins/chrome-essential-extensions/) 里也有性能相关的建议

如果关闭硬件加速后问题依旧，可能是显卡驱动之外的故障。检查 [Chrome 崩溃闪退修复指南](/tips/chrome-crash-fix/) 排查其他原因；有时候页面的视觉效果异常（如字体模糊、颜色偏移）也可能和[Chrome 深色模式](/tips/chrome-dark-mode/)的设置叠加有关，建议一并进行排查。
