...
不同平台的 Chrome 在性能表现上有本质差异，这直接影响你切换设备时的体验感受。

### 内存占用对比

**测试条件：** 同一设备（Windows 11 PC + 安卓手机 + iPhone 15），打开 10 个相同标签页（百度、淘宝、知乎、B站、GitHub、微博、京东、CSDN、豆瓣、YouTube），等待页面完全加载后测量。

| 指标 | 电脑版 | 安卓版 | iOS 版 |
|------|--------|--------|--------|
| 10 标签页总内存 | 1680MB | 约 800MB | 约 650MB |
| 单标签页平均 | 168MB | 80MB | 65MB |
| 后台标签页冻结 | ⚠️ 部分冻结 | ✅ 积极冻结 | ✅ iOS 系统级冻结 |
| 冷启动时间 | 1.2s | 0.8s | 0.7s |

**手机版内存显著更低**的原因有三个：一是移动端网页本身就比桌面版精简（响应式设计有时大幅删减内容），二是手机操作系统对后台进程限制更严格，三是 iOS 的 WKWebView 在内存管理上比 Blink 更激进。

### 电池续航影响

| 测试项目 | 电脑版 | 安卓版 | iOS 版 |
|---------|--------|--------|--------|
| 连续浏览 1 小时耗电 | 约 8%（笔记本） | 约 12% | 约 10% |
| 视频播放 1 小时耗电 | 约 15% | 约 22% | 约 18% |
| 后台静默耗电 | 极低 | 中等 | 低（iOS 限制更强） |

**结论：** iOS 版 Chrome 的耗电表现优于安卓版，这归功于 iOS 的严格后台管理和 WebKit 的能效优化。但如果你追求极致的手机续航，用 Safari 仍然是最省电的选择（Safari 比 Chrome iOS 版省电约 15-20%）。

### JavaScript 性能

使用 Speedometer 3.0 跑分测试（分数越高越好）：

| 测试 | 电脑版（V8 引擎） | 安卓版（V8 引擎） | iOS 版（JavaScriptCore） |
|------|-------------------|-------------------|--------------------------|
| Speedometer 3.0 | 28.5 | 22.3 | 24.1 |
| JetStream 2 | 178 | 125 | 142 |

**iOS 版用了 Apple 的 JavaScriptCore 引擎，在一些基准测试中比安卓版的 V8 得分还高。** 但这不意味着实际网页一定更快——实际体验取决于网页本身的优化程度和网络速度，差距通常在毫秒级别，用户几乎感觉不到。
...
Chrome 的跨平台同步能力在日常工作中能解决很多实际问题。以下是几个典型场景和对应的操作策略。

### 场景 1：电脑上查资料，手机上看

你在电脑上开了 10 个标签页做调研，要出门了怎么办？

**操作方法：** 手机 Chrome → 三点菜单 → 「最近打开的标签页」→ 能看到所有电脑上打开的标签页，点一下就能在手机上继续看。前提是电脑和手机都登录了同一个 Google 账号且同步功能可用。

**无同步时的替代方案：** 电脑上右键标签页 → 「发送到你的设备」，选择目标手机。这个功能走的是 Google 推送通道，国内同样不能直接使用。替代方法是用微信/Telegram 把链接发给自己，或使用第三方工具如 Pushbullet。

### 场景 2：手机上发现的网页，回到电脑继续处理

手机浏览器不适合做深度操作（填长表单、编辑文档、批量下载），但阅读和发现内容很高效。

**正向流程：** 手机 Chrome → 书签/历史记录 → 自动同步 → 电脑上 `chrome://bookmarks` 或 `chrome://history` 找到。因为手机端的输入效率远低于电脑，建议只在手机上做「标记」（加书签），回电脑后再处理。

### 场景 3：公共电脑上安全地用完就走

出差时用酒店或网吧的公共电脑，不能登录个人账号同步，但又想快速访问几个常用网站。

**操作方法：** 电脑版 Chrome 打开访客模式（点击头像 → 访客），关闭后所有浏览数据自动清除。如果必须在访客模式下查书签，建议用手机打开书签页面，然后用电脑上的访客模式手动输入网址——虽然慢，但绝对安全，不会在公共电脑上留下任何个人痕迹。

### 场景 4：电脑版负责内容创作，手机版负责内容消费

这是最自然的分工：电脑上编辑文档、写代码、处理表格，手机上阅读、看视频、浏览社交平台。

**关键配置：** 在电脑版中为工作相关的网站（如 GitHub、语雀、飞书文档）创建专门的标签页组，这些标签页组在手机上不可见，但能在电脑上快速恢复。手机版重点使用主页的推荐文章和地址栏的搜索功能，减少输入量。
