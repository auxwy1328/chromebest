...
Chrome 跑分领先约 8-10%。但跑分差距在 10% 以内，实际浏览体验的差距比跑分更小。视频播放方面，我在两个浏览器上播放同一个 4K 视频，Chrome 的 CPU 占用稳定在 12-15%，Firefox 在 18-22%。这是因为 Chrome 使用了硬件加速的视频解码，而 Firefox 的硬件解码支持相对有限。如果你经常看视频，这个差距会影响电池续航。
